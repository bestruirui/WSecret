const SHELL_CACHE = "wsecret-shell-v1";
const RUNTIME_CACHE = "wsecret-runtime-v1";
const CORE_ASSETS = [
...
self.addEventListener("activate", (event) => {
	event.waitUntil((async () => {
		...
		await Promise.all(
			cacheNames
				.filter((cacheName) => (cacheName.startsWith("wsecret-") || cacheName.startsWith("wrule-")) && cacheName !== SHELL_CACHE && cacheName !== RUNTIME_CACHE)
